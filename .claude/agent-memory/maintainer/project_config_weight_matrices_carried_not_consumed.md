...
direct WHO/JHU/SUPP = 1.0). They are injected POST `make_simulation_config` in
...
- `get_location_config.R` IS the only row-subset site that must keep weights row-aligned; it was
  ...
  `est_epidemic_peaks`/`make_simulation_config` `reported_cases[...]` refs are NOT the config matrix.
