...
in v0.69.0). Run R via `~/bin/r-mosaic-Rscript` (batch) or `~/bin/r-mosaic-R` (interactive). Plain
`Rscript` makes `check_dependencies()` report "BROKEN" and TensorFlow won't import. Simulation itself
no longer needs the wrapper (pure R since v0.68.0) — unverified on the VM. Invoke the wrapper **normally** (shebang repaired 2026-06-16; setup scripts now
