...
# =============================================================================
# Auxiliary function for testing with synthetic data
# =============================================================================
...
#' @param true_ess Target effective sample size pattern
#'
#' @return Data frame with synthetic results
#' @export
generate_test_data_for_ess <- function(n_samples = 2000, n_params = 5, true_ess = c(1500, 1000, 500, 200, 100)) {
    ...
    results <- data.frame(
        sim = 1:n_samples,
        iter = 1,
        seed_sim = 1:n_samples,
        seed_iter = 1:n_samples
    )
    
    # Generate parameters with varying levels of constraint
    for (i in 1:n_params) {
        if (i <= length(true_ess)) {
            # Create parameter with specific ESS characteristics
            ess_ratio <- true_ess[i] / n_samples
            
            # Generate clustered samples to simulate different ESS
            n_cluster <- round(n_samples * ess_ratio)
            cluster_centers <- runif(5, -2, 2)
            cluster_assignments <- sample(1:5, n_samples, replace = TRUE, 
                                          prob = c(0.4, 0.3, 0.2, 0.05, 0.05))
            
            param_vals <- rnorm(n_samples, 
                                mean = cluster_centers[cluster_assignments],
                                sd = 0.5 / sqrt(ess_ratio))
        } else {
            # Uniform sampling (high ESS)
            param_vals <- runif(n_samples, -3, 3)
        }
        
        results[[param_names[i]]] <- param_vals
    }
    
    # Generate likelihood based on weighted combination of parameters
    # Parameters with lower index have more influence (lower ESS)
    likelihood <- rowSums(sapply(1:n_params, function(i) {
        weight <- 1 / i
        -0.5 * weight * (results[[param_names[i]]])^2
    }))
    
    # Add noise
    likelihood <- likelihood + rnorm(n_samples, 0, 0.1)
