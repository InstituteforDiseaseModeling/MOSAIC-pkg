...
#' @param zero_buffer Logical; if \code{TRUE} (default), applies zero buffer to Poisson calculations
#'                    to handle non-integer observed data. If \code{FALSE}, enforces strict requirements.
                                  ...
                                  verbose = FALSE)
{
     ...
     for (j in seq_len(n_locations)) {
          ...
          if (zero_buffer) {
               ...
               est_cases_eval  <- pmax(est_cases[j, ],  1e-12)
               est_deaths_eval <- pmax(est_deaths[j, ], 1e-12)

          } else {

               est_cases_eval  <- est_cases[j, ]
               est_deaths_eval <- est_deaths[j, ]

          }

          # Calculate log-likelihood for cases
          if (family_cases == "poisson") {
               ll_cases <- MOSAIC::calc_log_likelihood(
                    observed  = obs_cases[j, ],
                    estimated = est_cases_eval,
                    family    = family_cases,
          ...
          ll_max_cases <- MOSAIC::calc_log_likelihood(
               ...
               estimated = max(est_cases_eval, na.rm = TRUE),
               family    = "poisson",
               weights   = NULL,
               verbose   = FALSE
          )
          ...
          ll_max_deaths <- MOSAIC::calc_log_likelihood(
               ...
               estimated = max(est_deaths_eval, na.rm = TRUE),
               family    = "poisson",
               weights   = NULL,
               verbose   = FALSE
          )
