#' Estimate, Visualize, and Impute WASH Coverage and Correlation with Cholera Incidence
#'
#' This function processes Water, Sanitation, and Hygiene (WASH) coverage data for African countries, optimizes the weights of WASH variables, calculates weighted means, examines their correlation with cholera incidence, and imputes missing WASH values using a similarity matrix. The function generates plots and tables showing these relationships and saves the results to specified paths.
#'
...
#'   \item \strong{DATA_SIMILARITY_MATRIX}: Path to the similarity matrix CSV file for imputation.
#' }
#'
#' @return The function does not return a value, but generates and saves plots and CSV files showing WASH coverage estimates, optimized weights for WASH variables, their correlation with cholera incidence, and imputed values for missing data.
#'
#' @details
...
#'   \item Imputes missing WASH data based on a country similarity matrix. Missing WASH values are estimated using the weighted average of the top 3 most similar countries.
#'   \item Generates multiple plots showing relationships between WASH variables and cholera incidence.
#'   \item Saves tables of optimized weights, weighted mean WASH indices, and imputed WASH values.
#' }
#'
...
est_WASH_coverage <- function(PATHS) {
     ...
     ################################################################################
     # Load the similarity matrix for imputation
     ################################################################################

     similarity_matrix_path <- file.path(PATHS$DATA_SIMILARITY_MATRIX, "similarity_matrix_africa.csv")
     if (!file.exists(similarity_matrix_path)) stop("Similarity matrix file not found.")

     similarity_matrix <- read.table(similarity_matrix_path, header = TRUE, row.names = 1)

     # Initialize a variable to hold the missing countries' imputed values
     missing_countries <- row.names(similarity_matrix)[!(row.names(similarity_matrix) %in% wash_data$iso_code)]
     missing_countries <- na.omit(missing_countries)

     imputed_countries <- data.frame()

     # Impute missing Weighted_Mean_WASH values based on the similarity matrix
     for (iso_code in missing_countries) {

          # Find the similar countries that have non-missing WASH data
          similar_countries <- unlist(similarity_matrix[iso_code, ])
          similar_countries <- similar_countries[complete.cases(similar_countries)]
          similar_countries <- similar_countries[names(similar_countries) %in% wash_data$iso_code]
          similar_countries <- sort(similar_countries, decreasing = TRUE)
          ...
          if (length(similar_countries) > 0) {
               ...
               tmp$Country <- MOSAIC::convert_iso_to_country(iso_code)
               tmp$iso_code <- iso_code
               tmp$Weighted_Mean_WASH <- imputed_value
               tmp$imputed <- 1

               imputed_countries <- rbind(imputed_countries, tmp)

               message(glue::glue("Imputed WASH value for {iso_code}: {imputed_value}"))

          } else {

               warning(glue::glue("No similar countries found to impute WASH value for {iso_code}."))
          }
     }


     wash_data$imputed <- 0
     wash_data <- rbind(wash_data, imputed_countries)
     ...
     wash_data <- wash_data[order(wash_data$Weighted_Mean_WASH, decreasing = TRUE),]
