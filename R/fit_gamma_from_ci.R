...
#' Test the gamma fitting function with vaccine effectiveness parameters
#'
#' @examples
#' \dontrun{
#' # Test with omega_1 values from the vaccine effectiveness analysis
...
#' library(ggplot2)
#' 
#' x_vals <- seq(0, 0.002, length.out = 1000)
#' df <- data.frame(
#'   x = rep(x_vals, 2),
#'   density = c(
#'     dgamma(x_vals, shape = omega_1_fit$shape, rate = omega_1_fit$rate),
#'     dgamma(x_vals, shape = omega_2_fit$shape, rate = omega_2_fit$rate)
#'   ),
#'   parameter = rep(c("omega_1", "omega_2"), each = length(x_vals))
#' )
#' 
#' ggplot(df, aes(x = x, y = density, color = parameter)) +
#'   geom_line(size = 1.5) +
#'   theme_minimal() +
#'   labs(title = "Fitted Gamma Distributions for Vaccine Waning Rates",
#'        x = "Waning rate (per day)",
#'        y = "Probability density")
#' }
test_gamma_fitting <- function() {
  cat("Testing gamma distribution fitting for vaccine parameters:\n")
  cat(paste(rep("=", 50), collapse=""), "\n")
  
  # Test omega_1
  cat("\nOmega_1 (one-dose waning):\n")
  omega_1_mm <- fit_gamma_from_ci(mode_val = 0.000705, 
                                   ci_lower = 0.000471, 
                                   ci_upper = 0.001107, 
                                   method = "moment_matching", verbose = TRUE)
  omega_1_opt <- fit_gamma_from_ci(mode_val = 0.000705, 
                                    ci_lower = 0.000471, 
                                    ci_upper = 0.001107, 
                                    method = "optimization", verbose = TRUE)
  
  # Test omega_2  
  cat("\nOmega_2 (two-dose waning):\n")
  omega_2_mm <- fit_gamma_from_ci(mode_val = 0.000337, 
                                   ci_lower = 0.0000984, 
                                   ci_upper = 0.000992, 
                                   method = "moment_matching", verbose = TRUE)
  omega_2_opt <- fit_gamma_from_ci(mode_val = 0.000337, 
                                    ci_lower = 0.0000984, 
                                    ci_upper = 0.000992, 
                                    method = "optimization", verbose = TRUE)
  ...
  cat("\n=== Method Comparison ===\n")
  cat("Omega_1:\n")
  cat(sprintf("  Moment matching: shape = %.2f, rate = %.0f\n", 
              omega_1_mm$shape, omega_1_mm$rate))
  cat(sprintf("  Optimization:    shape = %.2f, rate = %.0f\n", 
              omega_1_opt$shape, omega_1_opt$rate))
  
  cat("\nOmega_2:\n")
  cat(sprintf("  Moment matching: shape = %.2f, rate = %.0f\n", 
              omega_2_mm$shape, omega_2_mm$rate))
  cat(sprintf("  Optimization:    shape = %.2f, rate = %.0f\n", 
              omega_2_opt$shape, omega_2_opt$rate))
  
  invisible(list(omega_1_mm = omega_1_mm, omega_1_opt = omega_1_opt,
                 omega_2_mm = omega_2_mm, omega_2_opt = omega_2_opt))
}