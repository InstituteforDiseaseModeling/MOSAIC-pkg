...
laser_phase_susceptible <- function(state, par, ctl, tick) {
     ...
     # (`.astype(S_next.dtype)`), so coerce here -- .laser_pois returns a double
     # because the environmental sites overflow int32.
     births <- as.integer(.laser_pois(ctl, "susceptible/births",
...
#' @rdname laser_components
#' @keywords internal
laser_phase_exposed <- function(state, par, ctl, tick) {

     .laser_at(ctl, tick, "Exposed")
     here <- .row_at(tick)
     nxt  <- .row_next(tick)
     ...
     deaths <- .laser_binom(ctl, "exposed/non_disease_deaths",
...
#' @rdname laser_components
#' @keywords internal
laser_phase_recovered <- function(state, par, ctl, tick) {

     .laser_at(ctl, tick, "Recovered")
     here <- .row_at(tick)
     nxt  <- .row_next(tick)
     ...
     deaths <- .laser_binom(ctl, "recovered/non_disease_deaths",
     ...
     waned <- .laser_binom(ctl, "recovered/waning", r - deaths, par$waning_prob)
     r_next <- r_next - waned
...
#' @rdname laser_components
#' @keywords internal
laser_phase_infectious <- function(state, par, ctl, tick) {

     .laser_at(ctl, tick, "Infectious")
     here <- .row_at(tick)
     nxt  <- .row_next(tick)
     ...
     ndd <- .laser_binom(ctl, "infectious/sym_non_disease_deaths", is_next, ndd_prob)
     is_next <- is_next - ndd
     ...
     dd <- .laser_binom(ctl, "infectious/disease_deaths", is_next, -expm1(-mu_jt))
     ...
     if (idx_death_report >= 0L) {
          rep_d <- .laser_binom(ctl, "infectious/reported_deaths",
     ...
     rec_sym <- .laser_binom(ctl, "infectious/sym_recovery", is_next, par$gamma_1_prob)
     is_next <- is_next - rec_sym
     r_next <- state$R[[nxt]] + rec_sym

     # -- asymptomatic ----------------------------------------------------------
     ia_next <- state$Iasym[[here]]
     ndd_a <- .laser_binom(ctl, "infectious/asym_non_disease_deaths", ia_next, ndd_prob)
     ia_next <- ia_next - ndd_a
     ndd_total <- ndd_total + ndd_a       # same row as the symptomatic deaths

     rec_asym <- .laser_binom(ctl, "infectious/asym_recovery", ia_next, par$gamma_2_prob)
     ...
     e_next <- state$E[[nxt]]
     progressing <- .laser_binom(ctl, "infectious/progression", e_next, par$iota_prob)
     ...
     if (idx_probe >= 0L) {
          ...
          drawn <- .laser_binom(ctl, "infectious/reported_cases",
...
#' @rdname laser_components
#' @keywords internal
laser_phase_vaccinated <- function(state, par, ctl, tick) {

     .laser_at(ctl, tick, "Vaccinated")
     here <- .row_at(tick)
     nxt  <- .row_next(tick)
     ndd_prob <- par$non_disease_death_prob_jt[here, ]
     ...
     ndd1 <- .laser_binom(ctl, "vaccinated/v1_non_disease_deaths", v1_next, ndd_prob)
     v1_next <- v1_next - ndd1
     ndd_total <- state$non_disease_deaths[[here]] + ndd1

     ndd2 <- .laser_binom(ctl, "vaccinated/v2_non_disease_deaths", v2_next, ndd_prob)
     v2_next <- v2_next - ndd2
     ndd_total <- ndd_total + ndd2
     state$non_disease_deaths[[here]] <- ndd_total

     # -- waning vaccine-derived immunity --------------------------------------
     waned1 <- .laser_binom(ctl, "vaccinated/v1_waning", v1_next, par$omega_1_prob)
     v1_next <- v1_next - waned1
     s_next <- state$S[[nxt]] + waned1

     waned2 <- .laser_binom(ctl, "vaccinated/v2_waning", v2_next, par$omega_2_prob)
...
#' @rdname laser_components
#' @keywords internal
laser_phase_human_to_human <- function(state, par, ctl, tick) {

     .laser_at(ctl, tick, "HumanToHuman")
     here <- .row_at(tick)
     nxt  <- .row_next(tick)
     ...
     new_infections <- .laser_binom(ctl, "humantohuman/infection",
                                    local, -expm1(-rate))
...
#' @rdname laser_components
#' @keywords internal
laser_phase_env_to_human <- function(state, par, ctl, tick) {

     .laser_at(ctl, tick, "EnvToHuman")
     here <- .row_at(tick)
     nxt  <- .row_next(tick)
     ...
     new_infections <- .laser_binom(ctl, "envtohuman/infection",
                                    local_s, -expm1(-psi))
...
#' @rdname laser_components
#' @keywords internal
laser_phase_environmental <- function(state, par, ctl, tick) {

     .laser_at(ctl, tick, "Environmental")
     here <- .row_at(tick)
     nxt  <- .row_next(tick)
     ...
     decay <- pmin(.laser_pois(ctl, "environmental/decay",
     ...
     shed_sym <- .laser_pois(ctl, "environmental/shedding_sym",
                             par$zeta_1 * state$Isym[[here]], state$.npatches)
     w_next <- w_next + (1 - par$theta_j) * shed_sym

     shed_asym <- .laser_pois(ctl, "environmental/shedding_asym",
