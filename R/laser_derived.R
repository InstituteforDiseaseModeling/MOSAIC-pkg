...
#' @rdname laser_derived
#' @keywords internal
laser_phase_derived_values <- function(state, par, ctl, tick) {

     .laser_at(ctl, tick, "DerivedValues")
     ...
     S    <- .laser_rows(state$S, rows)
     Njt  <- .laser_rows(state$N, rows)
     Ijt  <- .laser_rows(state$Isym, rows) + .laser_rows(state$Iasym, rows)

     # Nothing here reaches an integer, so `1 - tau_i` stays in double rather
     # than going through `par$local_frac` -- see .laser_f32() for the rule.
     stay <- 1 - par$tau_i

     S_star <- .laser_by_patch(S, stay)
     ...
     # diagonal (laser_pi_ij) -- its own commented-out reference loop skips
     ...
     y_bar <- (.laser_by_patch(Ijt, stay) + Ijt %*% xfer_ij) / rowSums(Njt)
     ...
     y <- (.laser_rows(state$Isym) + .laser_rows(state$Iasym)) / .laser_rows(state$N)
     state$coupling <- .laser_coupling(y)
...
# list of per-tick vectors (see laser_alloc_state); this is the only component
# that needs whole-series arrays, and it needs them once.
.laser_rows <- function(series, rows = NULL) {
...
.laser_by_patch <- function(m, v) m * rep(v, each = nrow(m))
