...
laser_params <- function(config, components = LASER_PIPELINE) {
     ...
     if (any(c("Isym", "Iasym") %in% par$compartments)) {
          ...
          # `round(sigma * progressing)` -- see .laser_f32().
          par$sigma <- .laser_f32(
               .laser_scalar(config$sigma, "sigma", lower = 0, upper = 1))
     }
     ...
     if ("Recovered" %in% components) {
          par$epsilon     <- .laser_scalar(config$epsilon, "epsilon", lower = 0)
          par$waning_prob <- -expm1(-par$epsilon)
     }

     if ("Infectious" %in% components) {
          par$iota    <- .laser_scalar(config$iota,    "iota",    lower = 0)
          par$gamma_1 <- .laser_scalar(config$gamma_1, "gamma_1", lower = 0)
          par$gamma_2 <- .laser_scalar(config$gamma_2, "gamma_2", lower = 0)
          par$iota_prob    <- -expm1(-par$iota)
          par$gamma_1_prob <- -expm1(-par$gamma_1)
          par$gamma_2_prob <- -expm1(-par$gamma_2)

          par$rho        <- .laser_scalar(config$rho,        "rho",        lower = 0, upper = 1)
          par$rho_deaths <- .laser_scalar(config$rho_deaths, "rho_deaths", lower = 0, upper = 1)
          ...
          par$chi_endemic  <- .laser_f32(.laser_scalar(config$chi_endemic,
                                                       "chi_endemic",  positive = TRUE))
          par$chi_epidemic <- .laser_f32(.laser_scalar(config$chi_epidemic,
                                                       "chi_epidemic", positive = TRUE))

          par$delta_reporting_cases  <- .laser_lag(config$delta_reporting_cases,
                                                   "delta_reporting_cases")
          par$delta_reporting_deaths <- .laser_lag(config$delta_reporting_deaths,
          ...
          if (any(par$mu_j_baseline < 0)) {
               stop("`mu_j_baseline` is negative at patch(es) ",
                    .laser_fmt(which(par$mu_j_baseline < 0)), ".", call. = FALSE)
          }
          if (any(par$mu_j_epidemic_factor < 0)) {
               stop("`mu_j_epidemic_factor` is negative at patch(es) ",
                    .laser_fmt(which(par$mu_j_epidemic_factor < 0)), ".", call. = FALSE)
          }
          ...
          par$epidemic_threshold <- .laser_f32(.laser_patch_vector(
               config$epidemic_threshold, "epidemic_threshold", par$npatches,
               lower = 0))
     }

     if ("Vaccinated" %in% components) {
          par$omega_1 <- .laser_scalar(config$omega_1, "omega_1", lower = 0)
          par$omega_2 <- .laser_scalar(config$omega_2, "omega_2", lower = 0)
          par$omega_1_prob <- -expm1(-par$omega_1)
          par$omega_2_prob <- -expm1(-par$omega_2)
          # float32: each scales a dose count inside a `round()`.
          par$phi_1 <- .laser_f32(.laser_scalar(config$phi_1, "phi_1", lower = 0, upper = 1))
          par$phi_2 <- .laser_f32(.laser_scalar(config$phi_2, "phi_2", lower = 0, upper = 1))

          for (nm in c("nu_1_jt", "nu_2_jt")) {
               # float32: `round(nu_*_jt[tick])` is the delivered dose count.
               par[[nm]] <- .laser_f32(.laser_time_matrix(config[[nm]], nm,
                                                          par$nticks, par$npatches))
     ...
     if (any(c("HumanToHuman", "EnvToHuman", "DerivedValues") %in% components)) {
          ...
          # binomial `n`. Reproduce the stored precision; see .laser_f32().
          par$local_frac <- .laser_f32(1 - par$tau_i)
     }

     if ("HumanToHuman" %in% components) {
          # alpha_1 is scalar-or-per-patch in the engine and must be in (0, 1];
          # alpha_2 is a plain scalar.
          par$alpha_1 <- .laser_patch_vector(config$alpha_1, "alpha_1",
                                             par$npatches, upper = 1)
          if (any(par$alpha_1 <= 0)) {
               stop("`alpha_1` must be in (0, 1]; got a value <= 0 at patch(es) ",
                    .laser_fmt(which(par$alpha_1 <= 0)), ".", call. = FALSE)
          }
          par$alpha_2 <- .laser_scalar(config$alpha_2, "alpha_2", positive = TRUE)
     }

     if (any(c("EnvToHuman", "Environmental") %in% components)) {
          par$theta_j <- .laser_patch_vector(config$theta_j, "theta_j",
     ...
     if ("EnvToHuman" %in% components) {
          ...
          par$kappa <- .laser_scalar(config$kappa, "kappa", positive = TRUE)
     }

     if ("Environmental" %in% components) {
          par$zeta_1 <- .laser_scalar(config$zeta_1, "zeta_1", lower = 0)
          par$zeta_2 <- .laser_scalar(config$zeta_2, "zeta_2", lower = 0)
     }
...
# Reject it instead.
.laser_lag <- function(x, name) {
     x <- .laser_scalar(x, name, lower = 0)
     if (x != trunc(x)) {
