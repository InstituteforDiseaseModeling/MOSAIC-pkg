...
#'
#' @param suffix Filename suffix identifying which est_mobility() run to plot
#'   (e.g. \code{"_fused_raked_tt"}). Default \code{""} = the production
#'   air-derived fit.
...
plot_mobility <- function(PATHS, suffix = "") {

     # Mirrors est_mobility(suffix=): an air run writes mobility_*.png, a fused
     # run writes mobility_*_fused_raked_tt.png, so the two never collide.
     .in  <- function(f) .mosaic_suffix_path(file.path(PATHS$MODEL_INPUT, f), suffix)
     .fig <- function(f) .mosaic_suffix_path(file.path(PATHS$DOCS_FIGURES, f), suffix)
     ...
     lonlat <- utils::read.csv(.in("mobility_lon_lat.csv"))
     M <- as.matrix(read.csv(.in("mobility_M.csv"), header = TRUE, row.names = 1, stringsAsFactors = FALSE))
     D <- as.matrix(read.csv(.in("mobility_D.csv"), header = TRUE, row.names = 1, stringsAsFactors = FALSE))
     pi <- as.matrix(read.csv(.in("mobility_pi.csv"), header = TRUE, row.names = 1, stringsAsFactors = FALSE))

     N <- utils::read.csv(.in("mobility_N.csv"), header = TRUE, row.names = 1, stringsAsFactors = FALSE)
     tmp <- as.vector(N[,1])
     names(tmp) <- row.names(N)
     N <- tmp

     tau <- utils::read.csv(.in("mobility_tau.csv"))
     tmp <- as.vector(tau[,1])
     names(tmp) <- row.names(tau)
     tau <- tmp

     tau_df <- read.csv(.in("mobility_travel_prob_params.csv"), header = TRUE, row.names = 1, stringsAsFactors = FALSE)
     ...
     png(filename = .fig("mobility_flight_data.png"), width = 9, height = 9, units = "in", res = 300)
     print(p1)
     dev.off()
     ...
     png(filename = .fig("mobility_network.png"), width = 9, height = 9, units = "in", res = 300)
     ...
     # Breaks are DATA-DRIVEN, not hard-coded. The former fixed
     # c(0, 500, 2000, 5000) were tuned to the air-derived tau (median ~3e-5,
     # totals in the hundreds); under the ~12x larger overland tau totals reach
     # ~1.1e5, so those breaks spanned only ~4% of the axis and collapsed into an
     # unreadable overlapping clump at the left edge. Spacing evenly in SQRT space
     # matches the sqrt x-scale so the labels come out visually equidistant.
     .tt  <- tau_df$mean * tau_df$population
     # 4 breaks, not 5: panel B is a third of the figure width (rel_widths 2:1),
     # so five long labels overlap. Short-scale labels (7K, 30K) keep them apart.
     .brk <- (seq(0, 1, length.out = 4)^2) * max(.tt, na.rm = TRUE)
     .brk <- sort(unique(ifelse(.brk == 0, 0, signif(.brk, 1))))
     bar_plot <- ggplot(tau_df, aes(x = mean * population, y = reorder(iso3, mean))) +
          geom_bar(stat="identity", fill="dodgerblue") +
          scale_x_sqrt(breaks = .brk,
                       label = scales::label_number(scale_cut = scales::cut_short_scale())(.brk)) +
          theme_minimal() +
     ...
     png(filename = .fig("mobility_travel_prob_tau.png"), width = 7, height = 9, units = "in", res = 300)
     print(p3)
     dev.off()
     ...
     png(filename = .fig("mobility_diffusion_pi.png"), width = 9, height = 9, units = "in", res = 300)
     print(p4)
     dev.off()
