...
run_MOSAIC_dask <- function(config,
  ...
  # Run ensemble once via calc_model_ensemble(). The same object is used for
  # (a) R²/bias ratios, (b) windowed fit metrics, and (c) predictive plots when
  # plots = TRUE. Simulations run locally (Coiled cluster is already closed).
  ...
  if (n_ensemble_r2 > 1L) {
    log_msg("Computing ensemble R\u00b2 (%d stochastic runs, local)...", n_ensemble_r2)
