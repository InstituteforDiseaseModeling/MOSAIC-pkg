...
#'   diagonal.
#' @keywords internal
sim_distance_matrix <- function(latitude, longitude) {
...
#'   \code{npatches}.
#' @param d Distance matrix from \code{sim_distance_matrix()}.
...
#' @keywords internal
sim_pi_ij <- function(N, d, omega, gamma) {
...
sim_beta_jt_env <- function(par) {
     ...
     if (any(psi_bar == 0)) {
          stop(sprintf(paste0("psi_jt has a zero time-mean at patch(es) %s, so the ",
                              ...
                              "normalised this way."),
                       .sim_fmt(which(psi_bar == 0))), call. = FALSE)
     }
...
#' @keywords internal
sim_precompute <- function(par) {

     # `DerivedValues` needs `pi_ij` and `beta_jt_human` too; see sim_params().
     if (any(c("HumanToHuman", "DerivedValues") %in% par$components)) {
          d <- sim_distance_matrix(par$latitude, par$longitude)
          # The gravity model's populations are the INITIAL compartment sums,
          # not the running N -- pi_ij is built once and held constant.
          par$pi_ij <- sim_pi_ij(par$N_j_gravity, d, par$mobility_omega,
                                 par$mobility_gamma)
          par$beta_jt_human <- sim_beta_jt_human(par)
     }
     if ("EnvToHuman" %in% par$components) {
          par$beta_jt_env <- sim_beta_jt_env(par)
     }
     if ("Environmental" %in% par$components) {
          par$delta_jt <- sim_delta_jt(par)
     }
     par
}
