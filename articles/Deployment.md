...
Save this script as `setup_mosaic_vm.sh` and run via
`ssh user@host 'bash -s' < setup_mosaic_vm.sh`

This script installs R, system dependencies, MOSAIC package, and Python
components from scratch on Ubuntu/Debian systems.
...
For Azure-specific deployment, see the `azure/` directory in MOSAIC-pkg
for batch execution scripts.
