...
dir_output <- file.path(base_output_dir, paste0("ETH_adaptive_", run_stamp))
...
# (from: Running-MOSAIC vignette, adaptive algorithm example)
# ---------------------------------------------------------------------------
ctrl <- mosaic_control_defaults()

# Calibration: adaptive mode
ctrl$calibration$n_simulations   <- "auto"
ctrl$calibration$n_iterations    <- 5
ctrl$calibration$batch_size      <- 2000
ctrl$calibration$min_batches     <- 5
ctrl$calibration$max_batches     <- 20
ctrl$calibration$target_r2       <- 0.95
ctrl$calibration$max_simulations <- 1e+06

# Convergence targets
# ctrl$targets$ESS_param      <- 100
ctrl$targets$ESS_param      <- 500
