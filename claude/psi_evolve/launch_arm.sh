...
for i in $(seq 0 $((N-1))); do
  env PSI_ARM="$ARM" PSI_SEEDS="$SEEDS" PSI_SHARD="$i" PSI_NSHARD="$N" \
      PSI_GEOM="${PSI_GEOM:-}" PSI_STRIDE_DAYS="${PSI_STRIDE_DAYS:-84}" \
      PSI_LEAD="${PSI_LEAD:-0}" PSI_FEATURE_SET="${PSI_FEATURE_SET:-v7.3}" \
      PSI_SCREEN_EVERY="${PSI_SCREEN_EVERY:-1}" PSI_EXCLUDE="${PSI_EXCLUDE:-}" \
