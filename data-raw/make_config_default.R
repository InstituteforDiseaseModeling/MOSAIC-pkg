...
# time by sample_parameters() as CFR_target * gamma_1 * rho / (rho_deaths * chi).
# config_default still ships a numeric mu_j_baseline (the engine consumes it) AND
# a numeric CFR_target (the prior median, so sample_CFR_target=FALSE / NA-fallback
# paths have a default). Both are built here from the CFR_target lognormal prior
# MEDIAN exp(meanlog) and the STATIC chain anchor (the same anchor the B1 build
# used: gamma_1=0.10, chi=0.70, rho/rho_deaths from their Beta means), so the
# config-default mu_j_baseline reproduces the pre-B2 deterministic center while
# the SAMPLED mu (run_MOSAIC) couples to the per-draw gamma_1. The ETH-only dwell
# stop-gap is GONE (B2 subsumes it structurally). See statistician spec
# MOSAIC-pkg/claude/prior_fix_spec/SPEC_B2.md.
...
chi_anchor_cfg <- 0.70   # posterior-implied endemic/epidemic average
cfr_to_mu_adjustment_cfg <- g1_anchor_cfg * rho_mean_cfg / (rho_deaths_mean_cfg * chi_anchor_cfg)
...
message(sprintf(
     "  B2 config-default: mu_j_baseline = CFR_target * %.5f (static anchor gamma_1=%.2f chi=%.2f rho=%.4f rho_deaths=%.4f)",
     cfr_to_mu_adjustment_cfg, g1_anchor_cfg, chi_anchor_cfg, rho_mean_cfg, rho_deaths_mean_cfg))
...
# B2 (v4.5): per-country CFR_target (prior median). make_LASER_config() does not
