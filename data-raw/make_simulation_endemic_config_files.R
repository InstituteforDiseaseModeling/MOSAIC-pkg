# ---------------------------------------------------------------------------
# make_simulation_endemic_config_files.R
# ---------------------------------------------------------------------------
# Generate an *endemic* simulation parameter configuration for unit‑testing and
...
#
# Usage
#   source("make_simulation_endemic_config_files.R")
# ---------------------------------------------------------------------------
...
sim_args <- list(
     ...
     decay_days_long  = 365,       # vibrios can persist ≈ 1 yr in water
     decay_shape_1    = 1,
...
sim_config <- do.call(MOSAIC::make_simulation_config, sim_args)

# Tracking fields not accepted by make_simulation_config — inject after construction.
.zeta_ratio_sim        <- 3
.decay_days_spread_sim <- 364  # decay_days_long = short + spread
...
fp_json <- file.path(out_dir, "config_simulation_endemic.json")
...
params_validated <- do.call(MOSAIC::make_simulation_config, args2)
rm(args2)
...
message("Endemic simulation config written to:\n  - ",
