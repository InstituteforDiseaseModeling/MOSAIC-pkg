...
calc_npe_spec_v5_2 <- function(
    ...
    if (!is.null(n_timesteps) && n_timesteps > t_long_threshold) {
        ...
        # Ensure GroupNorm and attention head compatibility
        tcn_channels <- .ensure_groupnorm_divisibility(tcn_channels_new, attention_heads)
    ...
    if (!is.null(n_locations) && n_locations >= heads_J_threshold) {
        ...
        if (tcn_channels %% attention_heads != 0) {
            ...
            tcn_channels <- .ensure_groupnorm_divisibility(tcn_channels, attention_heads)
        }
...
#' @keywords internal
.ensure_groupnorm_divisibility <- function(channels, attention_heads = NULL) {
    ...
    for (g in rev(valid_groups)) {
        if (channels %% g == 0 && g <= channels) {
            # Also check attention head divisibility if provided
            if (!is.null(attention_heads) && channels %% attention_heads != 0) {
                next
            }
            return(channels)
        }
    ...
    for (g in c(8, 4, 2)) {
        if (g <= channels) {
            candidate <- g * ceiling(channels / g)
            # Ensure attention head divisibility if provided
            if (!is.null(attention_heads)) {
                candidate <- attention_heads * ceiling(candidate / attention_heads)
            }
            return(candidate)
        }
    }
    ...
    result <- 2 * ceiling(channels / 2)
    if (!is.null(attention_heads)) {
        result <- attention_heads * ceiling(result / attention_heads)
    }
    return(result)
}
