...
plot_model_posterior_quantiles <- function(csv_files,
    ...
    for (i in seq_along(csv_files)) {
        ...
        if (length(median_col) == 0) {
            # Fallback: look for any column near 0.5 (robust patterns)
            median_patterns <- c(
                "q50",        # q50 format
                "q0\\.50",    # q0.50 format
                "q0500",      # q0500 format (old NPE style)
                "q500",       # q500 format
                "median"      # explicit median column
            )
        ...
        # More robust parsing to handle various quantile formats
        quantile_vals <- numeric(0)
        valid_indices <- integer(0)

        for (i in seq_along(quantile_cols)) {
            col_name <- quantile_cols[i]
            # Try to extract numeric value from column name
            numeric_part <- gsub("^q", "", col_name)
            val <- suppressWarnings(as.numeric(numeric_part))
        ...
        if (length(quantile_vals) >= 2) {
            lower_idx <- valid_indices[which.min(quantile_vals)]
            upper_idx <- valid_indices[which.max(quantile_vals)]
            data$q_lower <- data[[quantile_cols[lower_idx]]]
            data$q_upper <- data[[quantile_cols[upper_idx]]]
        } else {
            # Fallback: use first and last quantile columns if numeric parsing fails
            if (length(quantile_cols) >= 2) {
                data$q_lower <- data[[quantile_cols[1]]]
                data$q_upper <- data[[quantile_cols[length(quantile_cols)]]]
                if (verbose) {
                    message(sprintf("Warning: Using first/last quantile columns as fallback for %s", basename(file_path)))
                }
            } else {
                stop(sprintf("Unable to identify lower/upper quantiles in %s. Available: %s",
                           basename(file_path), paste(quantile_cols, collapse = ", ")))
            }
        }
    ...
    if ("global" %in% plot_types) {
        ...
        if (nrow(global_data) > 0) {
            ...
            # Merge with estimated_parameters to get proper ordering and standardized descriptions
            # Use parameter_name from estimated_parameters to match with parameter column
            param_ordering <- estimated_parameters %>%
                select(parameter_name, description, order, category, scale) %>%
                rename(parameter = parameter_name)

            # Join to get standardized descriptions and ordering
            global_data <- global_data %>%
                left_join(param_ordering, by = "parameter", suffix = c("_original", "_standard"))

            # Use standardized description from estimated_parameters, fallback to original if missing
            global_data$description_final <- ifelse(
                is.na(global_data$description_standard),
                global_data$description_original,
                global_data$description_standard
            )

            # Create parameter labels with standardized descriptions
            global_data$param_label <- paste0(global_data$description_final, " (", global_data$parameter, ")")
