...
**Next action: Phase A-5 — calibration acceptance.** C-2 and the follow-on C-3 rename/cleanup pass are done: the package no longer declares, installs, imports, or checks `laser-cholera`, and the suite passes against an interpreter that does not have it. Everything in workstreams A and C is now complete except acceptance.

A-5 is Tier C across 4 configs x 200 seeds against §7's prespecified bands, preceded by the Python-vs-Python replicate run that establishes the noise floor those bands sit on, then one full local `run_MOSAIC()` calibration compared against the last Python-engine run on R², bias ratio, ESS and posterior marginals. **Note the ordering constraint that C-2 has just made awkward:** the Tier C noise floor and the Python-engine comparison run both need a working `laser-cholera`, which is no longer in `environment.yml`. The read-only v0.16.1 checkout at `/home/cliffk/idm/laser/laser-cholera` and the `mo` conda env still have it, so this is a matter of pointing the acceptance harness at that interpreter rather than at `r-mosaic` — but it should be set up deliberately, and the harness belongs in `claude/`, not in the package.

The remaining **A-3a** measurements are still owed and should land before A-5: the 1/10/40-patch scaling curve, sequential vs 20-worker throughput, peak RSS per worker, and end-to-end batch time. Peak RSS is the one that can force a code change — capping worker count by measured memory rather than `detectCores()` — and that cap would have to be in place before the acceptance calibration runs.
