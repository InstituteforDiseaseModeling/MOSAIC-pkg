...
**Next action: Phase A-5 — calibration acceptance.** C-2 is done: the package no longer declares, installs, imports, or checks `laser-cholera`, and the suite passes against an interpreter that does not have it. Everything in workstreams A and C is now complete except acceptance.
