...
- **C-2 — remove the LASER dependency.** Runs *after* the R engine is cut over in A-4, never before. **Done.**
...
|---|---|---|---|---|---|---|---|
| mean `calc_model_likelihood()`, relative | ±1% | 0.122 | 0.355 | 0.130 | 0.047 | **0.355** | 0.286 |
| mean `calc_model_likelihood()`, in σ | ±0.25 σ | 0.176 | 0.108 | 0.126 | 0.063 | **0.176** | 0.243 |
...
Measured 2026-09-11/12 on all four configs, Python repA (seeds 1:200) vs repB (seeds 1001:1200); the last column is the worst R-vs-Python result across the four. The acceptance band is `max(prior, 2 × floor)` per config, and **all 36 config × quantity bands pass**. Table generated by `claude/a5/floor_matrix.R` from the saved arm summaries rather than transcribed, because §7's band table is this document's load-bearing claim and lesson #15 is specifically about hand-built tables that pass a cardinality check while carrying wrong numbers.

**Two corrections to the first pass at this table.** (i) The 2026-09-11 run scored both arms with `calc_model_likelihood()` at its defaults, which omits the per-observation confidence-weight matrices that `run_MOSAIC()` passes straight through from the config — so it measured parity on an objective nobody calibrates against. With the real objective the floors drop and **the earlier finding that the ±0.25 σ prior band on the LL median was unachievable does not survive**: that floor is 0.181 σ, and every prior band is achievable on every config except LL p95 on the epidemic-threshold config (0.269 σ), where the floor-based recalibration is doing real work. (ii) Two quantities show a worst R-vs-Python larger than the worst floor — LL p05 (0.355 vs 0.213) and LL p95 (0.491 vs 0.269). Both were chased rather than passed on the band alone; see the replicate-pair analysis below.

**A floor estimated from one replicate pair is itself noisy, and on two configs that mattered.** The high-vaccination config's R-vs-Python KS screen came back *significant* (D = 0.155, p = 0.016) while every band passed, and the epidemic-threshold config produced the largest single R-vs-Python deviation anywhere in Tier C (LL p95 at 0.491 σ). Both were settled by adding a third Python replicate and a second R replicate and comparing every pairwise gap (`claude/a5/replicate_pairs.R`):
...
On both configs the difference *between* engines is smaller than Python's own arm-to-arm spread, and neither pooled test is close to significant. The D = 0.155 that triggered the investigation was the pairing of the highest-mean Python arm with the lowest-mean R arm out of five — a range statistic, not an engine difference. **On the high-vaccination config a within-Python pair also exceeds the KS critical value** (repB vs repC, D = 0.140 > 0.136, p = 0.040): Python fails the KS screen against itself there. That is direct empirical support for this section's insistence that KS is a screen and never the pass criterion, and it is the reason a max-to-max comparison of pair ranges was replaced by a pooled test — the range statistic is dominated by whichever two arms happen to sit at the extremes.
...
**Done, with one exit criterion unverified.** The suite was run against an interpreter with no `laser-cholera` importable (`RETICULATE_PYTHON=/software/conda/bin/python3`): **4,967 passing, 0 failures, 34 skips**, and `run_LASER()` returns its 28 channels on the 40-location 1,398-tick default config in 2.01 s under that interpreter. `R CMD check` on the built tarball is **3 WARNINGs / 3 NOTEs**, byte-identical to the A-4 baseline `claude/rcmdcheck7.log` apart from the timestamp and the version string (`claude/rcmdcheck8.log`). The unverified criterion is the clean `install_dependencies()` + `est_suitability()` training run: rebuilding the user's conda environment is destructive and was not done unasked. What *was* checked is that the new `environment.yml` parses to exactly `python=3.12, pip, numpy=1.26.4, packaging=24.0, numpy<2, tensorflow>=2.16.0,<2.21` with no laser entry, and that no R code imports any of the removed packages.

Three things came out of C-2 that were not in the plan:

1. **The `pkg_laser_cholera` decision went the other way from the plan's framing.** The plan worried about *not breaking* resume when the key disappears. The real problem was the opposite: the guard that read it compared the persisted version against `importlib.metadata.version("laser-cholera")` — whichever wheel happened to be installed, which after the A-4 cutover no longer described what had simulated anything. Once C-2 removed the wheel from `environment.yml`, that operand would have been permanently absent and the guard would have reported itself **SKIPPED on every single resume**: lesson #14(ii)'s dead guard, with a warning attached. But the hazard had grown rather than gone — a change of *engine* strictly contains a change of deaths scale. So the guard was re-keyed onto the MOSAIC version already recorded in `environment.json` (`.mosaic_run_engine()`, boundary at v0.66.0), which needs no Python at all, and resuming a pre-v0.66.0 run directory is now a hard error rather than a silent pooling of two simulators. `.mosaic_lc_pre013()` and `.mosaic_lc_deaths_scale()` went; their version-parsing and boundary-classification assertions were re-homed onto the replacement per lesson #14(iv) rather than deleted.

2. **Three more dead things fell out transitively** (lesson #14(iii), the orphan re-grep). `skip_if_no_python_likelihood()` had zero callers — the R-vs-Python likelihood parity tests it gated went with the Dask excision — and it was the *only* reader of the three capability flags that `tests/testthat/setup-python.R` probed eagerly, at a reticulate interpreter init plus two module imports (~6 s) **in every test process**. Both are gone, and with them the CI step whose stated purpose was to install the wheel so those tests would not skip. Separately, `.onLoad()`'s `NUMBA_THREADING_LAYER=workqueue` was setting an environment variable for a package that is no longer installed.

3. **`.mosaic_likelihood_provenance()` had a dead parameter.** C-1 reduced its body to a constant when scoring became R-only, but left the `lc_version` argument that every caller kept dutifully filling. Removed, with `expect_length(formals(...), 0L)` so it cannot creep back as a silently-ignored knob (lesson #13).

**Left for the user to decide, not done here:** the `hedgehog-run` and `dugong-run` skills and `vm/HEDGEHOG.md` / `vm/DUGONG.md` were corrected where they were factually wrong (dugong's "engine version" check and its wheel-bump recipe are gone) but **not retired**, since §6.2's "retire or reduce to a generic big-box note" is a call about the user's own workflows. The `azure/` tree (Dask/Coiled guides, Dockerfile, session logs) and `.claude/agent-memory/**` were left alone: the former documents a removed backend and the latter are historical records of past findings, so both want a deliberate pass rather than a find-and-replace. One concrete lead for the VM skills: dugong's `LD_PRELOAD` libexpat wrapper exists because PSOCK workers imported Python, and they no longer do — it may be unnecessary for `run_MOSAIC()` while remaining required for `est_suitability()`. Untested on the VM; noted in the skill.
...
> **Addendum (v0.71.0, 2026-09-15): the two sentences above are wrong, and the paragraph before them says why.** There *was* a second structural win of that size — 2.25x, larger than the 2.46x fix that prompted this paragraph is worth relative to what remained. The note above is correct that moving the container to an environment "changed nothing ... because fetching `M` bumps its reference count before the subassignment"; what was not carried through is that the *replacement* structure has the same property. The channels became lists of per-tick vectors but stayed on the state environment, so `state$S[[i]] <- v` remained a subassignment into an environment-held object and `[[<-` duplicated the whole `nticks + 1` pointer vector on every write. State is now one environment per tick, which is a pointer store with no long vector to copy. The engine is now **0.45 s** against the 1.183 s in the table above, so the 1.69x R-vs-Python ratio recorded here no longer holds. The flat profile was not evidence of absence: a per-function profiler charges allocation churn to the line that triggered it and to `<GC>`. See NEWS.md 0.71.0 and CLAUDE.md lesson 18.

> **Addendum 2 (v0.71.0, 2026-09-15): the ratio has now been re-measured, and it is the other way round.** Interleaved 6 blocks x 3 reps, R v0.71.0 against laser-cholera 0.16.1 in the `mo` conda env, both reading the SAME `inst/extdata/config_default.json`, both thread-pinned to 1, Python given an untimed warm-up so numba compilation is not charged to it (harness: `claude/perf/1e_xengine.sh` + `py_arm.py` + `1e_r_arm.R`, local laptop; raw rows in `claude/perf/1e_xengine.txt`). Per-block paired ratios, block minima:
>
...
>
> So R is now faster than the Python engine on the like-for-like path, not 1.69x slower, and the "compiled inner loop" conclusion below is moot as a *parity* argument — it would now be an argument for going faster than the thing we ported from, which is a different decision. Two caveats that matter for reading the table. (i) The spread within an arm across blocks (`r071_json` min 0.581, mean 0.723) is larger than the gap between the arms, which is exactly why the ratios are quoted per-block and paired and why the unpaired numbers higher up this document should not be compared against these. (ii) The JSON and in-memory arms differ by ~0.18 s of config parsing, against Python's 0.066 s `py_setup`. The R arm copies the config to a fresh temp file each rep specifically to defeat the v0.70.0 memoized reader, so 0.18 s is the cold-parse cost; in a calibration the reader is warm and the per-simulation cost is the `r071_rda` arm. Config parsing, not the tick loop, is now the largest remaining single non-engine cost on the cold path.

**This is a decision for the user, not an implementation detail.** (Superseded by Addendum 2 above: as of v0.71.0 R is 1.15-1.23x *faster* on the like-for-like path, so the price below is no longer being paid at all. The rest of this paragraph is kept as the record of the decision as it stood.) At 1.69x, a 40,000-simulation calibration costs about 13 CPU-hours against 8. Against that, the migration still removes reticulate, the ~2 GB-per-worker Python heap (R peaks near 700 MB), the 3.3 s per-worker import, the whole Python environment as a deployment dependency, and the orchestrator/worker version skew that silently invalidated the Coiled runs (issue #113). Whether 1.69x is an acceptable price for that is a judgement about priorities, and the honest position is that the plan oversold it by roughly a factor of 8.
...
**Next action: none in workstreams A or C — the migration is complete.** Every phase is done: the package no longer declares, installs, imports or checks `laser-cholera`, the engine is pure R, the `LASER` naming is gone, and acceptance has passed on all four Tier C configs (36/36 bands) and on the calibration comparison (32/32 bands). Two things remain outstanding, and neither blocks the port:

1. **A-3a parallel throughput still needs one clean run** on hedgehog or dugong. Everything else the gate asked for is measured and forces no code change — peak RSS is 926 MB/worker, flat from 1 to 19 workers, so memory does not cap worker count. Only the throughput curve is unmeasurable on a hybrid-topology laptop, and it was re-confirmed as unmeasurable here: the A-5 arms ran across a suspend, and the Python harness's wall-clock timer reported 125 s/sim for work that takes 0.9 s.
2. **`weighted_quantiles()` had a plotting-position bug — found here, now fixed in v0.69.1.** It interpolated against `cumsum(w)` (the upper edge of each weight block) instead of the midpoint `(cumsum(w) - 0.5w)/sum(w)`, biasing every weighted quantile downward in proportion to how concentrated the weights were. Its own roxygen example (`x = 1:5`, `w = c(.1,.2,.4,.2,.1)`, symmetric about 3) returned 2.5, and with 99% of the weight on `x = 2` it returned 1.49. Equal weights did not reproduce `stats::quantile()`, and splitting one weight across two copies of the same value changed the answer. Fixed, with regression tests pinning each violated invariance; `parity_tier2.rds` re-baselined with inputs asserted byte-identical, weighted means confirmed bit-identical at ~4e-16, and every changed cell moving up and none down. The A-5 calibration acceptance was re-run against the fixed function and is **unchanged at 32/32 bands** — as expected, since both arms were always reduced by the same function. Unrelated to the port; see NEWS 0.69.1.

**On the Python-engine comparison the plan asked for.** A-5's text specifies "one full local `run_MOSAIC()` calibration compared against the last Python-engine run". No archived Python-engine run is reachable from this machine — there is no MOSAIC root here, `get_paths()` fails, and neither VM resolves — and C-1 removed the `engine="python"` path that could regenerate one, so the comparison was inverted instead: the same 500 parameter draws through both engines, scored by the same R likelihood. This is the better-controlled experiment regardless of availability. An archived run would have differed in data vintage, priors version and package version as well as in engine, and those confounds cannot be separated after the fact; here the engine is the only thing that differs. Confirmed with Cliff before proceeding.
