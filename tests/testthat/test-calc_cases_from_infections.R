test_that("delta_t = 0 maps 1:1 with no NAs and correct scaling", {
     # These represent symptomatic infections (already filtered by sigma)
     infections <- c(0, 5, 20, 60) * 0.25  # equivalent to previous sigma * infections
     rho   <- 0.70
     chi_endemic  <- 0.50
     ...
     expect_equal(out$cases_true,      rho * infections, tolerance = 1e-12)
     expect_equal(out$cases_suspected, (rho / chi_endemic) * infections, tolerance = 1e-12)
})
...
test_that("delta_t > 0 pads with NAs and shifts correctly", {
     ...
     expect_equal(out$cases_true[(k+1):4],      rho * infections[1:(4-k)], tolerance = 1e-12)
     expect_equal(out$cases_suspected[(k+1):4], (rho / chi_endemic) * infections[1:(4-k)], tolerance = 1e-12)
})

test_that("epidemic switching uses chi_epidemic when infections/N exceeds threshold", {
     infections <- c(10, 50, 40, 10) * 0.25  # equivalent to previous sigma * infections
     N_vec <- rep(1000, length(infections))
     thr <- 0.03 * 0.25  # Adjust threshold for symptomatic infections (3% * 0.25)
     # infections/N = (0.0025, 0.0125, 0.01, 0.0025) -> flags: FALSE, TRUE, TRUE, FALSE
     rho <- 0.70
     ...
     expected_sus <- (rho / chi_use) * infections
...
test_that("When epidemic_threshold = NULL, N is ignored and chi_endemic is used everywhere", {
     infections <- c(5, 10, 15) * 0.25  # equivalent to previous sigma * infections
     out <- calc_cases_from_infections(
          infections = infections,
     ...
     expect_equal(out$cases_suspected, (0.70 / 0.60) * infections, tolerance = 1e-12)
})

test_that("delta_t >= length(infections) returns all NAs", {
     infections <- c(1, 2, 3) * 0.25  # equivalent to previous sigma * infections
     out <- calc_cases_from_infections(
          infections = infections,
          N = NULL,
          rho = 0.70,
