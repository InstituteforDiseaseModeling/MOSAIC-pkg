...
# test-removed_api.R
#
# Removed and renamed API surface must fail LOUDLY, never silently.
#
...
test_that("the Dask/Coiled helpers are gone entirely", {
  ...
  expect_false(exists("check_coiled_workspace", where = asNamespace("MOSAIC"),
                      inherits = FALSE))
  expect_false(exists("mosaic_dask_presets", where = asNamespace("MOSAIC"),
                      inherits = FALSE))
})

test_that("LASER-named functions error with a pointer to the new name", {
  # Renamed, not aliased: an alias that silently forwards is how a dead name
  # survives for years. These must say the new name out loud.
  expect_error(run_LASER(config = list()), "run_simulation")
  expect_error(run_laser(config = list()), "run_simulation")
  expect_error(make_LASER_config(), "make_simulation_config")
  expect_error(get_default_LASER_config(), "get_default_config")
  # ... and say WHY, so the reader is not left guessing whether behaviour moved.
  expect_error(run_LASER(config = list()), "renamed")
})

test_that("every renamed stub points at a function that actually exists", {
  # The failure this catches: renaming the stub's target later and leaving the
  # error message pointing at a name the package no longer has.
  for (new in unname(MOSAIC:::.MOSAIC_RENAMED_FUNS)) {
    expect_true(is.function(get(new, envir = asNamespace("MOSAIC"))),
                info = new)
  }
})
