...
This guide shows how to run single simulations using the `run_simulation()` function with deterministic parameters. `run_simulation()` is the pure-R transmission engine at the core of MOSAIC: a stochastic metapopulation SEIR model of cholera, with human and environmental transmission, vaccination and spatial coupling.
...
```{r epidemic-plot, echo = FALSE, out.width = "90%", fig.align = "center"}
...
```{r endemic-plot, echo = FALSE, out.width = "90%", fig.align = "center"}
...
```{r multi-location-example, echo=TRUE}
library(MOSAIC)
...
```{r multi-location-grid-plot, echo = FALSE, out.width = "100%", fig.align = "center"}
knitr::include_graphics("figures/multi_location_grid.png")
```
...
- **Function help**: `?run_simulation`, `?make_simulation_config`
