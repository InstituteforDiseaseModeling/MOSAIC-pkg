...
`run_LASER()` executes **one simulation per call**. For multiple simulations, use loops:
...
The `run_LASER()` function returns a Python object with results and metadata:
...
Use `make_LASER_config()` to create custom parameter sets:
...
**Note**: `make_LASER_config()` has 71+ parameters. Use `?make_LASER_config` for complete documentation.
...
To run models for specific countries, you can filter or create configurations:
...
**Solution**: Set a fixed seed: `run_LASER(paramfile = ..., seed = 123)`

**Issue**: Slow execution
**Solution**: Use parallel execution (see "Running Multiple Simulations" section)
...
- **Calibration**: For Bayesian calibration workflows, see calibration documentation
- **Parameter Estimation**: Explore `est_*` functions for data-driven parameters
