...
cases_epidemic <- as.matrix(model_epidemic$patches$expected_cases)
deaths_epidemic <- as.matrix(model_epidemic$patches$disease_deaths)

# Remove first row (time index) and transpose to [location × time]
cases_epidemic <- t(cases_epidemic[-1, ])
deaths_epidemic <- t(deaths_epidemic[-1, ])
...
# Extract time series results (convert Python objects to R matrices)
cases_endemic <- as.matrix(model_endemic$patches$expected_cases)
deaths_endemic <- as.matrix(model_endemic$patches$disease_deaths)

# Remove first row (time index) and transpose to [location × time]
cases_endemic <- t(cases_endemic[-1, ])
deaths_endemic <- t(deaths_endemic[-1, ])
