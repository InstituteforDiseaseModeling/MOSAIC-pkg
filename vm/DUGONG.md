...
pass without it, which masks the problem — use the wrapper anyway. (Since v0.68.0 simulation is pure R
and imports no Python, so the pyexpat conflict should now only reach the TensorFlow psi path. Unverified.) Recreate it with the tracked `vm/make_wrappers.sh` (`bash vm/make_wrappers.sh` on the VM) if lost.

The wrapper ALSO passes `--max-connections=512`. R fixes its connection table at startup (128 slots,
3 for stdin/stdout/stderr) and each PSOCK worker holds one, so a default R build caps a cluster at
~125 workers however many cores the box has — measured on dugong, `n_cores = 170` silently became
**123** for calibration and the ensemble cluster threw outright. The option is startup-only, so it
has to come from the wrapper; bare `Rscript` costs ~28% of the machine without erroring. A later
flag still wins if you need a different value.
